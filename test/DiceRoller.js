...
// const MockedDiceRoller = artifacts.require("mocks/MockedDiceRoller");
...
contract('DiceRoller validation', async (accounts) => {
  it('will fail if too many dice are used.', async () => {
    try {
      const numberOfDie = 11;
      const dieSize = 10;
      const adjustment = 0;
      const result = 13;
      let tx = await instance.hasRolled(numberOfDie, dieSize, adjustment, result);
      //console.log(tx)
      assert.fail(null, null, 'Should fail because too many dice.')
    }
    catch(err){
      // console.log(err.message)
      assert.isOk( err.message.indexOf('Too many dice') !== -1, "Expected too many dice error");
    }
  });
  ...
  it('will fail if too dice size is > 100.', async () => {
    try {
      ...
      let tx = await instance.hasRolled(numberOfDie, dieSize, adjustment, result);
      //console.log(tx)
      assert.fail(null, null, 'Should fail because dice size too large.')
    }
    catch(err){
      //console.log(err.message)
      assert.isOk( err.message.indexOf('100 sided die') !== -1, "Expected dice too large error");
    }
  });
  ...
  it('will fail if too large an adjustment is used.', async () => {
    try {
      ...
      let tx = await instance.hasRolled(numberOfDie, dieSize, adjustment, result);
      //console.log(tx)
      assert.fail(null, null, 'Should fail because adjustment size too large.')
    }
    catch(err){
      //console.log('Error: ' + err.message)
      assert.isOk( err.message.indexOf('Adjustment is too large') !== -1, "Expected Adjustment is too large error");
    }
  });  
  ...
  it('will fail if too small an adjustment is used.', async () => {
    try {
      ...
      let tx = await instance.hasRolled(numberOfDie, dieSize, adjustment, result);
      //console.log(tx)
      assert.fail(null, null, 'Should fail because adjustment size too small.')
    }
    catch(err){
      // console.log('Error: ' + err.message)
      assert.isOk( err.message.indexOf('Adjustment is too large') !== -1, "Expected Adjustment is too large error");
...
// contract('DiceRoller chainlink vfr', async (accounts) => {
//   it('will return a random number and call fulfillRandomness.', async () => {

//     try {
//       const mockedDiceRoller = await MockedDiceRoller.deployed()
//       const numberOfDie = 4;
//       const dieSize = 10;
//       const adjustment = 0;
//       const tx = await mockedDiceRoller.rollDice(numberOfDie, dieSize, adjustment);

//       // verify the DiceRolled event was emitted
//       assert.equal(tx.logs[0].event, "DiceRolled", "DiceRolled is expected to be emitted");

//       hasRolled = await mockedDiceRoller.hasRolledOnce( accounts[0] );
//       assert.equal(hasRolled, true, "Address should return true");
//     }
//     catch(err) {
